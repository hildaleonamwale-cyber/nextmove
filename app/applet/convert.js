...
body = body.replace(/onclick="([^"]*)"/g, (match, p1) => {
    if (p1.includes('switchTab')) {
        const tabMatch = p1.match(/'([^']+)'/);
        const tab = tabMatch ? tabMatch[1] : '';
        return `onClick={() => switchTab('${tab}')}`;
    }
    if (p1.includes('toggleSidebar')) return `onClick={toggleSidebar}`;
    if (p1.includes('toggleNotifications')) return `onClick={toggleNotifications}`;
    if (p1.includes('closeAuthModal')) return `onClick={() => setAuthModalActive(false)}`;
    if (p1.includes('closeListingPage')) return `onClick={() => setListingPageActive(false)}`;
    return `onClick={() => { /* ${p1} */ }}`;
});
...
// Fix specific unclosed tags or attributes
body = body.replace(/<br>/g, '<br />');
body = body.replace(/<hr>/g, '<hr />');

// Fix active tab logic
body = body.replace(/<a className="nm-nav-item active"/g, '<a className={`nm-nav-item ${activeTab === \'overview\' ? \'active\' : \'\'}`}');
body = body.replace(/<a className="nm-nav-item"/g, (match, offset, string) => {
    // This is a bit hacky, let's just replace all nav items
    return match;
});
